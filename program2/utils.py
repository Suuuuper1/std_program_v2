...
def data_visualizer(img, idx_to_class, path, bbox=None, pred=None):
    ...
    if bbox is not None:
        for bbox_idx in range(bbox.shape[0]):
            ...
            cv2.rectangle(
                img_copy,
                (one_bbox[0], one_bbox[1]),
                (one_bbox[2], one_bbox[3]),
                (255, 0, 0),
                2,
            )
...
def coord_trans(bbox, bbox_batch_idx, w_pixel, h_pixel, w_amap=7, h_amap=7, mode="a2p"):
    ...
    assert bbox.shape[-1] >= 4, (
        "the transformation is applied to the first 4 values of dim -1"
    )
    ...
    if mode == "p2a":
        # pixel to activation
        width_ratio = w_pixel[bbox_batch_idx] * 1.0 / w_amap
        height_ratio = h_pixel[bbox_batch_idx] * 1.0 / h_amap
...
def compute_iou(anchors, bboxes):
    ...
    # anchors: (M, 4) -> (M, 1, 4)
    # bboxes: (N, 4) -> (1, N, 4)
    anchors_expanded = anchors.unsqueeze(1)  # (M, 1, 4)
    bboxes_expanded = bboxes.unsqueeze(0)  # (1, N, 4)

    # 计算相交区域的坐标
    # 左上角取两个框的较大值
    inter_tl = torch.max(
        anchors_expanded[:, :, :2], bboxes_expanded[:, :, :2]
    )  # (M, N, 2)
    # 右下角取两个框的较小值
    inter_br = torch.min(
        anchors_expanded[:, :, 2:], bboxes_expanded[:, :, 2:]
    ...
    # 计算相交区域的宽高（如果无交集，宽高为负，后面会处理为0）
    inter_wh = inter_br - inter_tl  # (M, N, 2)
    # 将负值（无交集的情况）置为0
    inter_wh = torch.clamp(inter_wh, min=0)  # (M, N, 2)

    # 计算相交区域面积
    inter_area = inter_wh[:, :, 0] * inter_wh[:, :, 1]  # (M, N)

    # 计算anchors的面积
    anchors_wh = anchors[:, 2:] - anchors[:, :2]  # (M, 2)
    anchors_area = anchors_wh[:, 0] * anchors_wh[:, 1]  # (M,)

    # 计算bboxes的面积
    bboxes_wh = bboxes[:, 2:] - bboxes[:, :2]  # (N, 2)
    bboxes_area = bboxes_wh[:, 0] * bboxes_wh[:, 1]  # (N,)

    # 计算并集面积 = 两个框面积之和 - 交集面积
    union_area = (
        anchors_area.unsqueeze(1) + bboxes_area.unsqueeze(0) - inter_area
    )  # (M, N)

    # 计算IoU = 交集面积 / 并集面积
    iou = inter_area / union_area  # (M, N)
...
def compute_offsets(anchors, bboxes):
    ...
    """
    wh_offsets = torch.log(
        (bboxes[:, 2:4] - bboxes[:, :2]) / (anchors[:, 2:4] - anchors[:, :2])
    )

    xy_offsets = (
        bboxes[:, :2] + bboxes[:, 2:4] - anchors[:, :2] - anchors[:, 2:4]
    ) / 2.0
...
def generate_proposal(anchors, offsets):
    ...
    center_proposals = (anchors[:, :2] + anchors[:, 2:4]) / 2.0 + offsets[:, :2] * (
        anchors[:, 2:4] - anchors[:, :2]
    )

    wh_proposals = torch.exp(offsets[:, 2:4]) * (anchors[:, 2:4] - anchors[:, :2])

    top_left_proposals = center_proposals - wh_proposals * 0.5
    bottom_right_proposals = center_proposals + wh_proposals * 0.5

    proposals = torch.cat((top_left_proposals, bottom_right_proposals), dim=-1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
@torch.no_grad()
def assign_label(
    proposals, bboxes, background_id, pos_thresh=0.5, neg_thresh=0.5, pos_fraction=0.25
):
    """
    ...
    GT_class = torch.gather(box_cls, -1, max_iou_per_anc_ind.unsqueeze(-1)).squeeze(
        -1
    )  # M
    ...
    bboxes = torch.gather(
        bboxes_expand,
        -2,
        max_iou_per_anc_ind.unsqueeze(-1).unsqueeze(-1).expand(M, 1, 4),
    ).view(M, 4)
    ...
    negative_anc_ind = negative_anc_ind[
        torch.randint(0, negative_anc_ind.shape[0], (num_neg,))
    ]
